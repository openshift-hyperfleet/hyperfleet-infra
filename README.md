...
| Target | Description |
|--------|-------------|
| `make install-tracing` | Install Tempo + Grafana Alloy (OTel Collector) tracing backend |
| `make uninstall-tracing` | Uninstall Tempo + Grafana Alloy |
...
| `TRACING_ENABLED` | `false` | `false` | Set to `true` to deploy Tempo + Grafana Alloy and enable OTLP tracing in API/Sentinel/Adapter |
