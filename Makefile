...
# Override to test with a different chart version or forked repo:
#   make install-adapter1 ADAPTER_CHART_REF=main
#   make install-adapter1 CHART_ORG=myuser
CHART_ORG          ?= openshift-hyperfleet
API_CHART_REF      ?= $(API_IMAGE_TAG)
SENTINEL_CHART_REF ?= $(SENTINEL_IMAGE_TAG)
ADAPTER_CHART_REF  ?= $(ADAPTER_IMAGE_TAG)
...
define set-chart-ref
	@sed -i '' 's|github.com/[^/]*/|github.com/$(CHART_ORG)/|' $(1)/Chart.yaml
	@sed -i '' 's/\(?ref=\)[^"]*\"/\1$(2)"/' $(1)/Chart.yaml
...
help: ## Print available targets
	...
	@echo "  API_IMAGE_TAG      Image tag for API (default: v0.1.0)"
